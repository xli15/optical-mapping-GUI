function [fhtt, fhtr] = polt_nr_overtime_gui(ps_info, start_time,end_time,ps_thresh_low,ps_thresh_high,time_resoluation, Fs, bg )
...
for tt =1:length(lps_over_t)
    ...
    tend = min(tstart+time_resoluation,T_end);
        ...
        for iithr = 1:length(ps_thresh_low)
            ...
            loc1 = ps_info{1}( ((td_1 >=ps_thresh_low(iithr))& ...
            (td_1 < ps_thresh_high(iithr))),1:2);
            loc2 = ps_info{2}( ((td_2 >=ps_thresh_low(iithr))& ...
            (td_2 < ps_thresh_high(iithr))),1:2);
            temp_locs = [loc1;loc2];
            temp_loc_index = sub2ind(size(bg),temp_locs(:,2),temp_locs(:,1));
        
            lps_over_t(tt,iithr) = length(unique(temp_loc_index));
...
grid on
legend( 'Threshold setting 1','Threshold setting 2','Location','NorthEastOutside')
