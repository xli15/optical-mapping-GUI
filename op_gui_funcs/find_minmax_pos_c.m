function [minpos, maxpos]= find_minmax_pos_c(datapoint,win)
B = datapoint';
...
for ii =1:length(minpos)-1
    maxpos_temp = maxpos((maxpos>=minpos(ii))&(maxpos<=minpos(ii+1)));
    
    [~,ind] = max(datapoint(maxpos_temp));
    maxpos_new(ii) = maxpos_temp(ind);
end
